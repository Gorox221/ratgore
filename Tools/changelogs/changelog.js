...
log(`- GITHUB_TOKEN exists: ${!!process.env.GITHUB_TOKEN ? "YES (masked)" : "NO"}`);
if (process.env.GITHUB_TOKEN) {
  log(`- GITHUB_TOKEN first 8 chars: ${process.env.GITHUB_TOKEN.substring(0, 8)}...`);
}

// Use GitHub token if available for API calls
if (process.env.GITHUB_TOKEN) {
  log("Setting up axios with GitHub token authentication");
